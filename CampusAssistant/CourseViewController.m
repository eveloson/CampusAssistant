...
//    self.navigationItem.leftBarButtonItem = leftItem;
    ...
    //背景视图
    bgImageView = [[UIImageView alloc] initWithFrame:CGRectMake(0, 0, kWidth, kHeight)];
...
- (void)_initWeekView
{
    weekView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, ScreenWidth, ScreenHeight-20-44-49)];
    //初始化周视图的头
...
- (void)changeSubViewsFrameByWeek:(BOOL)_weekViewShow
{
    if (_weekViewShow) {
        //设置周课表视图以及其子视图的frame
        CGRect viewFrame = weekView.frame;
        viewFrame.origin.y -= 60;
        weekView.frame = viewFrame;
    } else {
